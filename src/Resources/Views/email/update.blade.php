...
<div class="clearfix">
  <table class="table borderless m-0">
    <tr>
      <td><div class="p-t-5">Title</div></td>
...
<div class="col-md-9 p-0">
  <div class="panel panel-default" data-sortable-id="ui-typography-7">
    ...
    <div class="panel-body p-5"> {!! Form::hidden('email_id',$email->id) !!}
      <table class="table borderless m-0">
        <tr>
          <td width="15%"><div class="p-5">From</div></td>
          <td width="85%">{!! Form::select('from_',$from, @$email->from_,['class'=>'form-control','id'=>'layout']) !!}</td>
        </tr>
        <tr>
          <td width="15%"><div  class="p-5">To</div></td>
          <td width="85%">@if(isset($settings['to_']) || $email->is_public==0 )
             <div class="bg-silver p-10">{!! ($email->to_!='')?$email->to_:'N/A'!!}</div>
            @else
            <div class="input-group">
              <input name="to_" class="form-control hide" data-tagit="tagit" type="text" value="{!! $email->to_ !!}">
              <div class="input-group-addon addonNone" data-toggle="tooltip" data-placement="right" title="{!! $to !!}"><i class="fa fa-info-circle fa-lg" aria-hidden="true"></i></div>
            </div>
            @endif </td>
        </tr>
        
         <tr>
          <td width="15%"><div  class="p-5">Notify To</div></td>
          <td width="85%"> 
            @if(isset($settings['notify_to']) || $email->is_public==0 )
              <div class="bg-silver p-10">{!! ($email->notify_to!='')?$email->notify_to:'N/A'!!}</div>
            @else
            <div class="input-group">
              <input name="notify_to" class="form-control hide" data-tagit="tagit" type="text" value="{!! $email->notify_to !!}">
              <div class="input-group-addon addonNone" data-toggle="tooltip" data-placement="right" title="{!! $to !!}"><i class="fa fa-info-circle fa-lg" aria-hidden="true"></i></div>
            </div>
            @endif
        ...
        <tr>
          <td width="15%"><div  class="p-5">CC</div></td>
          <td width="85%"> 
           @if(isset($settings['cc']) || $email->is_public==0 )
              <div class="bg-silver p-10">{!! ($email->cc!='')?$email->cc:'N/A'!!}</div>
           @else
            <div class="input-group">
              <input name="cc" class="form-control hide" data-tagit="tagit" type="text" value="{!! $email->cc !!}">
              <div class="input-group-addon addonNone" data-toggle="tooltip" data-placement="right" title="{!! $to !!}"><i class="fa fa-info-circle fa-lg" aria-hidden="true"></i></div>
            </div>
             @endif
        ...
        <tr>
          <td width="15%"><div  class="p-5">BCC</div></td>
          <td width="85%"> 
           @if(isset($settings['bcc']) || $email->is_public==0 )
              <div class="bg-silver p-10">{!! ($email->bcc!='')?$email->bcc:'N/A'!!}</div>
           @else
            <div class="input-group">
              <input name="bcc" class="form-control hide" data-tagit="tagit" type="text" value="{!! $email->bcc !!}">
              <div class="input-group-addon addonNone" data-toggle="tooltip" data-placement="right" title="{!! $to !!}"><i class="fa fa-info-circle fa-lg" aria-hidden="true"></i></div>
            </div>
            @endif 
            </td>
         ...
         <tr>
          <td width="15%"><div  class="p-5">Reply To</div></td>
          <td width="85%"> 
        ...
        <tr>
          <td width="15%"><div class="p-5">Subject</div></td>
          <td width="85%">{!! Form::text('subject',$email->subject,['class' => 'form-control', 'id' => 'subject' ,'placeholder' => 'subject']) !!}</td>
        </tr>
        <tr>
          <td width="15%"><div class="p-5">Attachment</div></td>
          <td width="85%">
             <button class="btn btn-default" type="button" data-role="browseMediabutton">Browse Media</button>
              <span class="m-l-10">{!! class_basename($email->attachment) !!}</span>
          	 <input class="form-control fileParth" type="hidden" hidden="hidden" name="attachment"  value="{!! $email->attachment !!}">
          </td>
        </tr>
        
        
        <tr id="editor">
          <td valign="top"><div class="p-5">Content</div></td>
          <td>
...
<div class="col-md-3 p-0 p-l-5">
  <div class="panel panel-default" data-sortable-id="ui-typography-7">
    ...
    <div class="panel-body p-5">
      <table class="table borderless m-0">
        <tr>
          <td><div class="p-b-5">Content Type</div>
        ...
        <tr class="template">
          <td><div class="p-b-5">Templates</div>
            {!! Form::select('template_id',$templates,$email->template_id,['class'=>'form-control','id'=>'template']) !!} </td>
        </tr>
        <tr class="template-var" style="display:none">
          <td><div class="p-b-5">Variations</div>
            {!! Form::select('variation_id',$variations,$email->variation_id,['class'=>'form-control','id'=>'variation_id']) !!} </td>
        </tr>
      </table>
    </div>
  </div>
  <div class="panel panel-default" data-sortable-id="ui-typography-7">
    ...
    <div class="panel-body p-5">
      <table class="table borderless m-0">
        ...
        <tr>
          <td><div class="p-b-5">Event / Trigger</div>
        ...
        <tr data-container="afterday" class="hide">
          <td><div class="p-b-5">After Days</div>
            {!! Form::select('custom_days', array('1' => '1 Day', '3' => '3 Days','5' => '5 Days' ,'10' => '10 Days','15' => '15 Days','30' => '30 Days','0' => 'Custom Date'), $email->custom_days, ['class'=>'form-control','id'=>'afterday', 'data-change'=>'settime']) !!} </td>
        </tr>
        <tr data-container="settime" class="hide">
          <td><div class="p-b-5">Select Date</div>
            <div class='input-group date' data-actions="Timercalendar">
              <input name="custom_time" class="form-control" type="text"  value="{!!  $email->custom_time !!}">
              <span class="input-group-addon"> <i class="fa fa-calendar" aria-hidden="true"></i> </span> </div></td>
        </tr>
      </table>
    </div>
  </div>
...
<style>
   	.input-group-addon.addonNone { background:none; border:0; box-shadow:none}
   </style>
@stop
